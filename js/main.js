...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /* ---------- Gallery Generation & Filter ---------- */
  if (typeof galleryData !== 'undefined' && galleryData.length > 0) {
    const masonryGrid = document.getElementById('masonryGrid');
    const galleryFilters = document.getElementById('galleryFilters');
    
    if (masonryGrid && galleryFilters) {
      const catNames = {
        'toys': 'Toys',
        'ballon': 'Balloons',
        'chocolate': 'Chocolates',
        'chocolate2': 'Wafers',
        'prices': 'Price Stickers',
        'stickers-cards': 'Cards',
        'stickersz and cards': 'Stickers & Cards',
        'Chocolate': 'Chocolates',
        'sriammantoys': 'Sri Amman Toys'
      };

      const uniqueCats = [...new Set(galleryData.map(item => item.category))];
      
      uniqueCats.forEach(cat => {
        const btn = document.createElement('button');
        btn.className = 'gfilter';
        btn.dataset.filter = cat;
        btn.textContent = catNames[cat] || (cat.charAt(0).toUpperCase() + cat.slice(1));
        galleryFilters.appendChild(btn);
      });

      galleryData.forEach(item => {
        const gItem = document.createElement('div');
        gItem.className = 'g-item';
        gItem.dataset.cat = item.category;
        const fallbackName = catNames[item.category] || item.category;
        gItem.innerHTML = `<img src="${item.path}" loading="lazy" alt="${fallbackName} at Sri Amman Toys" onerror="imgFallback(this,'${fallbackName}')">`;
        masonryGrid.appendChild(gItem);
      });
    }
  }
